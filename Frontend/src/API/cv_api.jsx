    ...
    try {
        const response = await axios.post(`${BASE_URL}/generate/cv`, formData, {
            headers: {
                ...
                'Authorization': `Bearer ${sessionStorage.getItem('token')}` // Assuming you need token for auth
            }
        });
        ...
        console.error('Error submitting CV data:', error.response?.data || error.message);
